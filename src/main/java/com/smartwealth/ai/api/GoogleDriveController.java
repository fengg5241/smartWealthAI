...
@RestController
@RequestMapping("/api/sync/google-drive")
public class GoogleDriveController {
    ...
    private static final String NOT_CONFIGURED_HTML = """
            ...
            <h2 style="color:#dc2626;">&#9888; Google Drive Not Configured</h2>
            <p style="color:#64748b;">Set <code>GOOGLEDRIVE_ENABLED=true</code> and configure Google Drive OAuth credentials in your environment, then restart the service.</p>
    ...
    public ResponseEntity<String> startAuth(@RequestParam(value = "tenantId", required = false) String tenantIdParam) {
        ...
        String authUrl = syncService.getAuthUrl(tenantId, state);

        return ResponseEntity.ok("""
                <html><body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;padding:40px;text-align:center;max-width:560px;margin:0 auto;">
                <h2 style="color:#2563eb;">Link Google Drive</h2>
                ...
                <li>Create a <strong>SmartRAG</strong> folder in your Google Drive</li>
                ...
                <p style="font-size:13px;color:#64748b;margin-top:12px;">SmartRAG will <strong>never</strong> access files outside the SmartRAG folder, and will never delete or modify anything in your Drive.</p>
                </div>
                <a href="%s" style="display:inline-block;padding:12px 32px;background:#2563eb;color:white;border-radius:8px;text-decoration:none;font-weight:600;font-size:15px;">Continue to Google</a>
                <p style="font-size:12px;color:#64748b;margin-top:16px;">You will be redirected to Google to sign in and authorize.</p>
                </body></html>""".formatted(authUrl));
    }
    ...
    public ResponseEntity<String> oauthCallback(@RequestParam(value = "code", required = false) String code,
        ...
        if (tenantId == null) {
            return ResponseEntity.badRequest().body("<html><body><h2>Invalid state — cannot determine tenant</h2></body></html>");
        }

        String result = syncService.exchangeCode(tenantId, code);
        // Trigger immediate sync to create the folder & import files
        if (result != null && result.startsWith("Google Drive connected")) {
    ...
    public ResponseEntity<Map<String, Object>> disconnect(
            @RequestParam(value = "tenantId", required = false) String tenantIdParam) {
        if (!available()) {
            return ResponseEntity.badRequest().body(Map.of("error", "Google Drive is not enabled"));
        }
        String tenantId = resolveTenant(tenantIdParam);
        if (tenantId == null) {
            return ResponseEntity.badRequest().body(Map.of("error", "Missing tenant"));
